...
static error_t parse_opt (int key, char *arg, struct argp_state *state)
{
	...
	case 'b':
		...
		if (baud_temp != EINVAL && baud_temp != ERANGE)
			...
			default:
				arguments->baudrate = BOTHER;
				arguments->other_rate = baud_temp;
				break;
			}
...
void write_midi_action_to_serial_port(snd_seq_t* seq_handle)
{
	...
	do
	{
		...
		int midi_len;
		snd_midi_event_new 	(MIDI_BUFSIZE, &midi_ev	);
		midi_len = snd_midi_event_decode (midi_ev, midi_buf, MIDI_BUFSIZE, ev);
		if (midi_len > 0) {
			write(serial, midi_buf, midi_len);
		}
...
void* read_midi_from_serial_port(void* seq)
{
	...
	while(!msg_full) {
		read(serial, &midi_byte, 1);
		msg_full=snd_midi_event_encode_byte(midi_ev, midi_byte, &ev );
	}
...
main(int argc, char** argv)
{
	 ...
	 */
	if (arguments.baudrate == BOTHER)
	{
		ioctl(serial, TCGETS2, &newtio);
		newtio.c_cflag &= ~CBAUD;
		newtio.c_cflag |= BOTHER;
		newtio.c_ospeed = arguments.other_rate;
		newtio.c_ispeed = arguments.other_rate;
