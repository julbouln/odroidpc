...
uint32_t usb_send_serial_data(void *buf, int len) {
    return usbd_ep_write_packet(usbdev, ENDPOINT_CDC_DATA_IN, buf, len);
}
...
uint32_t usb_send_keyboard_report(void) {
    if (keyboard_protocol) {
        return usbd_ep_write_packet(
            usbdev, ENDPOINT_KEYBOARD_HID_IN, &nkro_key_report, 32);
    } else {
        ...
        return usbd_ep_write_packet(usbdev, 
                                    ENDPOINT_KEYBOARD_HID_IN, 
                                    &boot_key_report, 
...
void usb_setup()
{
    ...
    usbdev = usbd_init(&st_usbfs_v2_usb_driver, &device_descriptor,
