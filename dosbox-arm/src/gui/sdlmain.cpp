...
dosurface:
	...
	case SCREEN_TEXTURE:
	{
		...
		sdl.texture.texture = SDL_CreateTexture(sdl.renderer, SDL_GetWindowPixelFormat(sdl.window),
...
*/
		...
		if (!sdl.texture.texture) {
			LOG_MSG("SDL:Can't create SDL_PIXELFORMAT_RGB888 texture");
...
static void GUI_StartUp(Section * sec) {
...
#if (defined C_OPENGL) && (defined USE_OPENGLES) // OpenGL ES
...
#if SDL_VERSION_ATLEAST(2,0,0)
	int nviddrv=SDL_GetNumVideoDrivers();
	int i;
	for(i=0;i<nviddrv;i++) {
		const char *viddrv=SDL_GetVideoDriver(i);
		printf("SDL: driver available %s\n",viddrv);
	}
#endif
   ...
   if(sdl.desktop.want_type==SCREEN_OPENGL){ /* OPENGL is requested */
	...
	LOG_MSG("SDL:Current window pixel format: %s/%d", SDL_GetPixelFormatName(sdl.desktop.sdl2pixelFormat),sdl.desktop.bpp);
